 ...
 /**
  ...
  function get_member(){
	 ...
	 foreach($members as $r){
		...
		$cha =array ('(',')','-');
		$sp =array ('','','');
		$phone =  str_replace($cha,$sp,$ph);
	 
	 $res[$r->id] = $r->title.' '.$r->first_name.' '.$r->last_name.' - ('.$phone.')';
	 }
  ...
  function get_member_email(){
	 ...
	 $res[$r->id] = $r->title.' '.$r->first_name.' '.$r->last_name.' - ('.$r->email.')';
	 }
  ...
  function list_users(){
	 ...
	 $this->select_all_key('users');
	 $members = $this->db->where($this->dx('active').'=1',NULL,FALSE)->get('users')->result();
   ...
   * @author Phil Sturgeon
   * */
  public function get_single_member($id = false)
  {
   $this->select_all_key('members');
    $res = $this->db->where('id',$id)->get('members')->row();
   ...
   * @author Phil Sturgeon
   * */
  public function get_single_visitor($id = false)
  {
  
    $res = $this->db->where('id',$id)->get('visitors')->row();
   

    return $res;
  }
  
  function get_ss_school(){
	
	 $ss = $this->db->get('sunday_school')->result();
