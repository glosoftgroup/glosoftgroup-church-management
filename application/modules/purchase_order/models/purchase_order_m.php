...
//This count unpaid
         ...
         function count_lpo_month()
         {
              ...
              return $this->db->where("DATE_FORMAT(FROM_UNIXTIME(created_on),'%m-%Y')", date('m-Y'))->where('status', 1)->count_all_results('purchase_order');
         }
         ...
         function suppliers()
         {
              $result = $this->db->where(array('address_book' => 'suppliers'))->get('address_book')->result();
              $r = array();
              foreach ($result as $res)
              {
                   $r[$res->id] = $res->business_name . ' - (' . $res->phone . ')';
              }
              return $r;
         }
