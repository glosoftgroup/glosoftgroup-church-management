<?php

    class Ministries_m extends MY_Model
    {

         function __construct()
         {
              // Call the Model constructor
              parent::__construct();
              $this->db_set();
         }

         function create($data)
         {
              $this->db->insert('ministries', $data);
              return $this->db->insert_id();
         }

         //Generated code

         function get_members($id)
         {
              $results = $this->db->where(array('ministry_id' => $id))
                           ->get('member_ministries')
                           ->result();

              return $results;
         }

         function find($id)
         {
              return $this->db->where(array('id' => $id))->get('ministries')->row();
         }

         function exists($id)
         {
              return $this->db->where(array('id' => $id))->count_all_results('ministries') > 0;
         }

         function exists_mem_min($id)
         {
              return $this->db->where(array('id' => $id))->count_all_results('member_ministries') > 0;
         }

         function count()
         {

              return $this->db->count_all_results('ministries');
         }

         function update_attributes($id, $data)
         {
              return $this->db->where('id', $id)->update('ministries', $data);
         }

         function populate($table, $option_val, $option_text)
         {
              $query = $this->db->select('*')->order_by($option_text)->get($table);
              $dropdowns = $query->result();
              $options = array();
              foreach ($dropdowns as $dropdown)
              {
                   $options[$dropdown->$option_val] = $dropdown->$option_text;
              }
              return $options;
         }

         function delete($id)
         {
              return $this->db->delete('ministries', array('id' => $id));
         }

         function delete_mem_min($id)
         {
              return $this->db->delete('member_ministries', array('id' => $id));
         }

         /**
          * Setup DB Table Automatically
          * 
          */
         function db_set()
         {
              $this->db->query(" 
	CREATE TABLE IF NOT EXISTS  ministries (
	id INT(11) NOT NULL AUTO_INCREMENT PRIMARY KEY ,
	code  varchar(256)  DEFAULT '' NOT NULL, 
	name  varchar(256)  DEFAULT '' NOT NULL, 
	leader  varchar(32)  DEFAULT '' NOT NULL, 
	telephone  varchar(256)  DEFAULT '' NOT NULL, 
	mobile  varchar(256)  DEFAULT '' NOT NULL, 
	email  varchar(256)  DEFAULT '' NOT NULL, 
	congregation_size  varchar(256)  DEFAULT '' NOT NULL, 
	description  text  , 
	created_by INT(11), 
	modified_by INT(11), 
	created_on INT(11) , 
	modified_on INT(11) 
	) ENGINE=InnoDB  DEFAULT CHARSET=utf8; ");
         }

         function paginate_all($limit, $page)
         {
              $offset = $limit * ( $page - 1);

              $this->db->order_by('id', 'desc');
              $query = $this->db->get('ministries', $limit, $offset);

              $result = array();

              foreach ($query->result() as $row)
              {
                   $result[] = $row;
              }

              if ($result)
              {
                   return $result;
              }
              else
              {
                   return FALSE;
              }
         }

    }
    