...
class Sangar_scaffolds
{
	...
	//page number  variable
	 \$data['page'] = \$page;
        ...
        /**
                *Add New " . humanize($this->controller_name) . " 
                *
                *@param \$page
                */
...
";
        ...
        if ($this->scaffold_model_type === 'phpactiverecord')
        {
            ...
            {
                    \$this->session->set_flashdata('message', array( 'type' => 'success', 'text' =>'" . humanize($this->controller_name) . " '. lang('web_create_success') ));
            }

            if ( $" . $this->model_name . "->is_invalid() )
            {
                    \$this->session->set_flashdata('error', array( 'type' => 'error', 'text' => \$ok->errors->full_messages() ));
            }
        ...
        else if ($this->scaffold_model_type === 'activerecord')
        {
            ...
            {
                    \$this->session->set_flashdata('error', array( 'type' => 'error', 'text' =>'" . humanize($this->controller_name) . " '. lang('web_create_failed') ));
            }
        ...
        $data .= (($this->there_is_a_relational_field) ? "redirect(\"admin/" . $this->controller_name . "/\".\$this->input->post('" . $this->relational_field . "', TRUE));" : "redirect('admin/" . $this->controller_name . "/');") . "
		 ...
		 \$this->template->title('Add " . humanize($this->controller_name) . " ' )->build('admin/create', \$data);
                     ...
                     /**
                *Edit  " . humanize($this->controller_name) . " 
                *
                *@param \$id
                *@param \$page
                */
	...
	{ 
        ...
        if ($this->there_is_an_image or $this->there_is_a_file)
        {
            ...
            $data .= "
                    \$data['" . $this->model_name . "'] = \$this->" . $this->model_name . "->find(\$id);
...
";
        ...
        if ($this->there_is_an_image or $this->there_is_a_file)
        {
            ...
            {
                $data .= "
					if ( is_file(FCPATH.'uploads/" . $this->controller_name . "/'.\$index) )
						unlink(FCPATH.'uploads/" . $this->controller_name . "/'.\$index);

					if ( is_file(FCPATH.'uploads/" . $this->controller_name . "/thumbs/'.\$index) )
						unlink(FCPATH.'uploads/" . $this->controller_name . "/thumbs/'.\$index);";
            }

            if ($this->there_is_a_file)
            {
                $data .= "
					if ( is_file(FCPATH.'uploads/" . $this->controller_name . "/files/'.\$index) )
						unlink(FCPATH.'uploads/" . $this->controller_name . "/files/'.\$index);";
            }
            $data .= "
				}
";
        ...
        $data .= "
				\$this->session->set_flashdata('message', array( 'type' => 'success', 'text' =>'" . humanize($this->controller_name) . " '. lang('web_edit_success') ));
...
";
		...
		{
			\$this->session->set_flashdata('message', array( 'type' => 'sucess', 'text' =>'" . humanize($this->controller_name) . " '. lang('web_delete_success') ));";
        ...
        if ($this->there_is_an_image or $this->there_is_a_file)
        {
            ...
            if ($this->there_is_an_image)
            {
                $data .= "
				if ( is_file(FCPATH.'uploads/" . $this->controller_name . "/'.\$index) )
					unlink(FCPATH.'uploads/" . $this->controller_name . "/img/'.\$index);

				if ( is_file(FCPATH.'public/uploads/" . $this->controller_name . "/thumbs/'.\$index) )
					unlink(FCPATH.'public/uploads/" . $this->controller_name . "/thumbs/'.\$index);";
            }
...
function populate(\$table,\$option_val,\$option_text)
{
  ...
  <div class=\"col-sm-12\">
        ...
        <div class=\"panel-heading\">
            ...
            <h3 class=\"panel-title\"><?php echo   (\$updType == 'edit') ? 'Edit ' : 'Add ';?> " .   humanize($this->controller_name) . " </h3>

              <div class=\"panel-tools\">
               <div class=\"btn-group\">
              <?php echo anchor( 'admin/" . $this->controller_name . "/create' , '<i class=\"icon-plus-sign-alt\"></i> <span> '.lang('web_add_t', array(':name' => '" . humanize($this->controller_name) . "')).'</span>', 'class=\"btn btn-primary\"');?> 
              <?php echo anchor( 'admin/" . $this->controller_name . "' , '<i class=\"icon-list\"></i> <span>'.lang('web_list_all', array(':name' => '" . humanize($this->controller_name) . "')).'</span>', 'class=\"btn btn-info\"');?> 
              </div>
            </div>
        </div>
                        ...
                        foreach ($this->languages as $prefix => $language)
                        {
                            $data .="
                                        ...
                                        <div class='form-group'>
                                                <label class=' col-sm-3 control-label' for='" . $index . "_" . $prefix . "'>" . humanize($index) . " ($prefix) " . (($value['required'] == "TRUE") ? "<span class='required'>*</span>" : "") . "</label><div class=\"col-sm-5\">
...
";
                    ...
                    else
                    {
                        $data .="
                                                <div class='form-group'>
                                                        <label class=' col-sm-3 control-label' for='" . $index . "'>" . humanize($index) . " " . (($value['required'] == "TRUE") ? "<span class='required'>*</span>" : "") . "</label><div class=\"col-sm-5\">
                                                        <?php echo form_input('" . $index . "' ,\$result->" . $index . " , 'id=\"" . $index . "_" . $prefix . "\"  class=\"form-control\" ' );?>
                                                        <i style=\"color:red\"><?php echo form_error('" . $index . "'); ?></i>
                                                </div>
                                                </div>
                                                ";
                    }
                ...
                case 'textarea':
                    ...
                    if ($value['multilanguage'] == "TRUE")
                    {
                        foreach ($this->languages as $prefix => $language)
                        {
                            $data .="
<<div class='form-group'>>
	<label class=' col-sm-3 control-label' for='" . $index . "_" . $prefix . "'>" . humanize($index) . " ($prefix) " . (($value['required'] == "TRUE") ? "<span class='required'>*</span>" : "") . "</label>
	<div class=\"col-sm-5\">
    <textarea id=\"" . $index . "_" . $prefix . "\"  class=\"autosize-transition ckeditor form-control\"  name=\"" . $index . "_" . $prefix . "\"  /><?php echo set_value('" . $index . "_" . $prefix . "', (isset(\$result->" . $index . "_" . $prefix . ")) ? htmlspecialchars_decode(\$result->" . $index . "_" . $prefix . ") : ''); ?></textarea>
	<?php echo form_error('" . $index . "_" . $prefix . "'); ?>
</div>
</div>
";
...
<div class='form-group'>
	<label class=' col-sm-3 control-label' for='" . $index . "'>" . humanize($index) . " " . (($value['required'] == "TRUE") ? "<span class='required'>*</span>" : "") . "</label><div class=\"col-sm-5\">
	<textarea id=\"" . $index . "\"  class=\"autosize-transition ckeditor form-control \"  name=\"" . $index . "\"  /><?php echo set_value('" . $index . "', (isset(\$result->" . $index . ")) ? htmlspecialchars_decode(\$result->" . $index . ") : ''); ?></textarea>
	<i style=\"color:red\"><?php echo form_error('" . $index . "'); ?></i>
</div>
</div>
";
                ...
                case 'checkbox':
                    $data .= "
<div class='form-group'>
	<div class=\"col-sm-5\"><input id='" . $index . "' " . (($value['checked'] == "TRUE") ? ' checked ' : '') . "type='checkbox' name='" . $index . "' value='1'  class=\"form-control\" <?php echo preset_checkbox('" . $index . "', '1', (isset(\$result->" . $index . ")) ? \$result->" . $index . " : ''  )?> />&nbsp;<label class=' col-sm-3 control-labelinline' for='" . $index . "'>" . $value['label'] . " " . (($value['required'] == "TRUE") ? "<span class='required'>*</span>" : "") . "</label>
	<i style=\"color:red\"><?php echo form_error('" . $index . "'); ?></i>
</div>
</div>
";
                ...
                case 'select':
                    $data .="
<div class='form-group'>
	<label class='col-sm-3 control-label' for='" . $index . "'>" . humanize($index) . " " . (($value['required'] == "TRUE") ? "<span class='required'>*</span>" : "") . "</label>
<div class=\"col-sm-5\">";
                ...
                <?php \$items = array('' =>'', ";
                    ...
                    $data .= $this->sl . ");		
     echo form_dropdown('" . $index . "', \$items" . ',  (isset($result->' . $index . ")) ? \$result->" . $index . " : ''     ,   ' id=\"form-field-select-1\" class=\"form-control search-select\" data-placeholder=\"Select Options...\" ');
    ?> <i style=\"color:red\"><?php echo form_error('" . $index . "'); ?></i>
</div></div>
";
                    break;
...
<div class='form-group'>
	<label class=' col-sm-3 control-label' for='" . $index . "'>" . humanize($index) . " " . (($value['required'] == "TRUE") ? "<span class='required'>*</span>" : "") . "</label>
	<div class=\"col-sm-5\">
    <?php echo form_dropdown('" . $index . "', \$" . strtolower(str_replace('_m', '', $value['options']['model'])) . ',  (isset($result->' . $index . ")) ? \$result->" . $index . " : ''     ,   ' id=\"form-field-select-1\" class=\"form-control search-select\" data-placeholder=\"Select  Options...\" ');
                            ?>";
...
";
                ...
                case 'radio':
                    $data .= "
<div class='form-group'>
	<label class='col-sm-3 control-label'>" . humanize($index) . " " . (($value['required'] == "TRUE") ? "<span class='required'>*</span>" : "") . "</label><div class=\"col-sm-5\">";

                    $c = 0;
                    foreach ($value['options'] as $index2 => $value2)
                    {
                        $data .= $this->sl . $this->tab . "<input type='radio' name='" . $index . "' id='" . $index . "_$c' value='" . $value2['value'] . "' <?php echo preset_radio('" . $index . "', '" . $value2['value'] . "', (isset(\$result->" . $index . ")) ? \$result->" . $index . " : '" . $value['checked'] . "'  );?> > <label class=' col-sm-3 control-labelinline' for='" . $index . "_$c'> " . $value2['label'] . " </label>";
                        $c++;
                    }
...
<div class='form-group'>
	...
	<input id='" . $index . "' type='text' name='" . $index . "' maxlength='' class='form-control date-picker' value=\"<?php echo set_value('" . $index . "', (isset(\$result->" . $index . ")) ? \$result->" . $index . " : ''); ?>\"  />
	<i style=\"color:red\"><?php echo form_error('" . $index . "'); ?></i>
                ...
                case 'image':
                    ...
                    if ($value['multilanguage'] == "TRUE")
                    {
                        foreach ($this->languages as $prefix => $language)
                        {
                            $data .= "
<div class='form-group'>
	<label class=' col-sm-3 control-label' for='" . $index . "_" . $prefix . "'><?php echo lang( (\$updType == 'edit')  ? \"web_image_edit\" : \"web_image_create\" )?> [" . $index . " ($prefix)] " . (($value['required'] == "TRUE") ? "<span class='required'>*</span>" : "") . "</label>
<div class=\"col-sm-5\">
	<?php if (\$updType == 'edit'): ?>
		<p> <img src='uploads/img/thumbs/<?php echo \$result->" . $index . "_" . $prefix . "?>' /> </p>
	<?php endif ?>
...
";
                    ...
                    else
                    {
                        $data .= "
<div class='form-group'>
	<label class=' col-sm-3 control-label' for='" . $index . "'><?php echo lang( (\$updType == 'edit')  ? \"web_image_edit\" : \"web_image_create\" )?> (" . $index . ") " . (($value['required'] == "TRUE") ? "<span class='required'>*</span>" : "") . "</label>
<div class=\"col-sm-5\">
	<?php if (\$updType == 'edit'): ?>
		 <img src='uploads/img/thumbs/<?php echo \$result->" . $index . "?>' /> 
	<?php endif ?>

	<input id='" . $index . "' type='file'  class=\"span5\" name='" . $index . "' />

	<br/><i style=\"color:red\"><?php echo form_error('" . $index . "'); ?></i>
                ...
                case 'file':
                    ...
                    if ($value['multilanguage'] == "TRUE")
                    {
                        foreach ($this->languages as $prefix => $language)
                        {
                            $data .= "
<div class='form-group'>
	<label class=' col-sm-3 control-label' for='" . $index . "_" . $prefix . "'><?php echo lang( (\$updType == 'edit')  ? \"web_file_edit\" : \"web_file_create\" )?> [" . $index . " ($prefix)] " . (($value['required'] == "TRUE") ? "<span class='required'>*</span>" : "") . "</label>
<div class=\"col-sm-5\">
	<input id='" . $index . "_" . $prefix . "' type='file'  class=\"form-control\" name='" . $index . "_" . $prefix . "' />

	<?php if (\$updType == 'edit'): ?>
		<p> <a href='uploads/files/<?php echo \$result->" . $index . "_" . $prefix . "?>' />Download actual file " . $index . " (" . $prefix . ")</a> </p>
	<?php endif ?>
...
";
                    ...
                    else
                    {
                        $data .= "
<div class='form-group'>
	<label class=' col-sm-3 control-label' for='" . $index . "'><?php echo lang( (\$updType == 'edit')  ? \"web_file_edit\" : \"web_file_create\" )?> (" . $index . ") " . (($value['required'] == "TRUE") ? "<span class='required'>*</span>" : "") . "</label>
 <div class=\"col-sm-5\">
	<input id='" . $index . "' type='file' name='" . $index . "' />

	<?php if (\$updType == 'edit'): ?>
	<a href='uploads/files/<?php echo \$result->" . $index . "?>' />Download actual file (" . $index . ")</a>
	<?php endif ?>
...
<div class='form-group'><label class=\"col-sm-3 control-label\"></label><div class=\"col-sm-5\">
	...
	<?php echo anchor('admin/" . $this->controller_name . "','Cancel','class=\"btn btn-light-grey\"');?>
</div></div>
...
<div class=\"clearfix\"></div>
  ...
  <div class=\"col-sm-12\">
        ...
        <div class=\"panel-heading\">
		<i class=\"icon-external-link-sign\"></i>
            <h3 class=\"panel-title\">" . lang('web_list_all', array(':name' => humanize($this->controller_name))) . " </h3>
        ...
        foreach ($this->arrayjson as $index => $value)
        {
            switch ($value['type'])
            {
                ...
                case 'image':
                    ...
                    if ($value['multilanguage'] == "TRUE")
                    {
                        $data .= $this->tabx5 . "<td><img src='uploads/img/thumbs/<?php "
                ...
                case 'file':
                    ...
                    if ($value['multilanguage'] == "TRUE")
                    {
                        ...
                        foreach ($this->languages as $prefix => $language)
                        {
                            $data .= $this->tabx6 . "<a href='uploads/files/<?php echo \$p->" . $index . "_" . $prefix . "?>'/>Download " . $index . " (" . $prefix . ")</a></br></br>" . $this->sl;
                        }
                        $data .= $this->tabx5 . "</td>";
                    }
                    else
                    {
                        $data .= $this->tabx5 . "<td><a href='uploads/files/<?php echo \$p->" . $index . "?>' />Download file (" . $index . ")</a></td>" . $this->sl;
                    }
                    break;
            }
        }
			...
			<div class='btn-group'>
				...
				<ul role='menu' class='dropdown-menu pull-right'>
					<li role='presentation'>
						<a role='menuitem' style='color:green' tabindex='-1' href='<?php echo site_url('admin/" . $this->controller_name . "/edit/'.\$p->id.'/'.\$page);?>'>
							<i class='icon-edit'></i> Edit
						</a>
					</li>
					<li role='presentation'>
						<a role='menuitem' style='color:green' tabindex='-1' href='<?php echo site_url('admin/" . $this->controller_name . "/view/'.\$p->id.'/'.\$page);?>'>
							<i class='icon-share'></i> View
						</a>
					</li>
					<li role='presentation'>
						<a role='menuitem' tabindex='-1' style='color:red' onClick=\"return confirm('<?php echo lang('web_confirm_delete')?>')\" href='<?php echo site_url('admin/" . $this->controller_name . "/delete/'.\$p->id.'/'.\$page);?>'>
